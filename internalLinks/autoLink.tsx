...
export const autoLinkReactNodes = (text: string): React.ReactNode[] => {
  if (!text) return [];
...
export const processNodesForLinking = (children: React.ReactNode): React.ReactNode => {
  return React.Children.map(children, (child) => {
    if (typeof child === 'string') {
      return autoLinkReactNodes(child);
    }
    ...
    if (React.isValidElement(child)) {
      ...
      if (element.props.children) {
        return React.cloneElement(element, {
          ...element.props,
          children: processNodesForLinking(element.props.children)
        });
      }
    }
    return child;
