  ...
  return (
    <div className="max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-32 md:pb-20">
      ...
      <div ref={stepContainerRef} className="space-y-6 min-h-[300px]">
        {/* Step 1-3 logic identical to original, just compressed for brevity here as they didn't change functionality... */}
        ...
        {step === 2 && (<div className="space-y-6 animate-in slide-in-from-right-4 duration-300"><div ref={el => { fieldRefs.current['time'] = el; }}><h3 className="text-sm font-bold text-slate-900 mb-3 uppercase tracking-wide">Time from Last Known Well</h3><div className="grid grid-cols-1 gap-3"><SelectionCard title="Early Window (0 - 6 Hours)" description="Uses NCCT (ASPECTS) or CTP." selected={inputs.time === '0_6'} onClick={() => updateInput('time', '0_6')} /><SelectionCard title="Late Window (6 - 24 Hours)" description="Requires Perfusion Imaging (DAWN/DEFUSE-3)." selected={inputs.time === '6_24'} onClick={() => updateInput('time', '6_24')} /></div></div><div ref={el => { fieldRefs.current['nihss'] = el; }} className="pt-4"><h3 className="text-sm font-bold text-slate-900 mb-3 uppercase tracking-wide">NIH Stroke Scale</h3><div className="grid grid-cols-2 gap-3"><SelectionCard title="0 - 5" description="Mild" selected={inputs.nihss === '0_5'} onClick={() => updateInput('nihss', '0_5')} /><SelectionCard title="6 - 9" description="Moderate" selected={inputs.nihss === '6_9'} onClick={() => updateInput('nihss', '6_9')} /><SelectionCard title="10 - 19" description="Mod-Severe" selected={inputs.nihss === '10_19'} onClick={() => updateInput('nihss', '10_19')} /><SelectionCard title="≥ 20" description="Severe" selected={inputs.nihss === '20_plus'} onClick={() => updateInput('nihss', '20_plus')} /></div></div></div>)}
        {step === 3 && (<div className="space-y-6 animate-in slide-in-from-right-4 duration-300">{inputs.time === '0_6' && (<div ref={el => { fieldRefs.current['aspects'] = el; }}><div className="bg-blue-50 p-4 rounded-xl text-blue-900 text-sm mb-6 border border-blue-100"><h4 className="font-bold flex items-center mb-2"><Info size={16} className="mr-2"/> Early Window Imaging</h4><p>Enter ASPECTS score from non-contrast CT.</p></div><label className="block text-sm font-bold text-slate-700 mb-2">ASPECTS Score (0-10)</label><input type="number" min="0" max="10" className="w-full p-4 text-lg bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-neuro-500 outline-none font-bold" placeholder="e.g. 8" value={inputs.aspects} onChange={(e) => updateInput('aspects', e.target.value)} /></div>)}{inputs.time === '6_24' && (<div><div className="bg-purple-50 p-4 rounded-xl text-purple-900 text-sm mb-6 border border-purple-100"><h4 className="font-bold flex items-center mb-2"><Info size={16} className="mr-2"/> Perfusion Imaging Required</h4><p>Enter values from automated software (RAPID, Viz.ai, etc).</p></div><div className="space-y-4"><div ref={el => { fieldRefs.current['core'] = el; }}><label className="block text-sm font-bold text-slate-700 mb-2">Ischemic Core Volume (ml)</label><input type="number" className="w-full p-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-neuro-500 outline-none font-medium" placeholder="CBF < 30%" value={inputs.core} onChange={(e) => updateInput('core', e.target.value)} /></div><div className="grid grid-cols-2 gap-4"><div ref={el => { fieldRefs.current['mismatchVol'] = el; }}><label className="block text-sm font-bold text-slate-700 mb-2">Mismatch Volume (ml)</label><input type="number" className="w-full p-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-neuro-500 outline-none font-medium" placeholder="Tmax > 6s volume" value={inputs.mismatchVol} onChange={(e) => updateInput('mismatchVol', e.target.value)} /></div><div ref={el => { fieldRefs.current['mismatchRatio'] = el; }}><label className="block text-sm font-bold text-slate-700 mb-2">Mismatch Ratio</label><input type="number" step="0.1" className="w-full p-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-neuro-500 outline-none font-medium" placeholder="Ratio" value={inputs.mismatchRatio} onChange={(e) => updateInput('mismatchRatio', e.target.value)} /></div></div></div></div>)}</div>)}

        {/* STEP 4: DECISION - UPDATED WITH AUTO-LINKING */}
        {step === 4 && result && (
