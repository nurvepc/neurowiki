...
type Inputs = {
  ...
  time: TimeWindow;
  nihss: NIHSSGroup;
  aspects: string; // number string
  core: string; // number string
  mismatchVol: string; // number string
  mismatchRatio: string; // number string
};
...
const calculateEvtProtocol = (inputs: Inputs): Result => {
  ...
  if (inputs.lvo === 'no') {
    return {
      eligible: false, status: "Not Eligible",
      reason: "No Large Vessel Occlusion (LVO)",
      details: "Thrombectomy is indicated for occlusions of the ICA or MCA (M1).",
      exclusionReason: "Absence of LVO target."
    };
  }
  if (inputs.mrs === 'no') {
    return {
      eligible: false, status: "Not Eligible",
      reason: "Pre-stroke Disability (mRS > 1)",
      details: "Standard criteria require pre-stroke functional independence.",
      exclusionReason: "Poor baseline functional status."
    };
  }
  if (inputs.age === 'under_18') {
    return {
      eligible: false, status: "Consult",
      reason: "Pediatric Patient",
      details: "Standard guidelines apply to age ≥ 18. Pediatric thrombectomy requires specialized consultation.",
      exclusionReason: "Age < 18."
    };
  }
  ...
  if (inputs.time === '0_6') {
     ...
     if (inputs.nihss === '0_5') {
         return {
             eligible: true, status: "Clinical Judgment",
             reason: "Low NIHSS (< 6)",
             details: "Guidelines recommend EVT if deficit is disabling despite low score (e.g., aphasia, hemianopsia).",
             criteriaName: "Early Window (Low NIHSS)"
         };
     }
     ...
     if (aspects >= 6) {
         return {
             ...
             criteriaName: "Standard Early Window",
             reason: "ASPECTS ≥ 6",
             details: "Class I evidence for benefit. Proceed to thrombectomy."
         };
     } else if (aspects >= 3) {
         return {
             eligible: true, status: "Eligible",
             criteriaName: "Large Core Protocol",
             reason: "ASPECTS 3-5",
             details: "SELECT2 and ANGEL-ASPECT trials demonstrate benefit for large core infarcts (ASPECTS 3-5). CAUTION: Higher hemorrhage risk. Requires multidisciplinary discussion and risk vs. benefit conversation."
         };
     } else {
         return {
             eligible: false, status: "Consult",
             reason: "Malignant Profile (ASPECTS 0-2)",
             details: "Very large core. High risk of futile reperfusion and hemorrhage. Individualized decision.",
             exclusionReason: "ASPECTS < 3"
         };
     }
  }
  ...
  if (inputs.time === '6_24') {
      ...
      // DAWN Criteria Logic
      // Group A: Age ≥ 80, NIHSS ≥ 10, Core < 21
      // Group B: Age < 80, NIHSS ≥ 10, Core < 31
      // Group C: Age < 80, NIHSS ≥ 20, Core 31-50
      ...
      if (nihssNum >= 10) {
          if (isAge80Plus) {
              if (core < 21) dawnEligible = true;
          } else {
              if (core < 31) dawnEligible = true;
              else if (core < 51 && nihssNum >= 20) dawnEligible = true;
          }
      }
      ...
      if (dawnEligible) {
          return {
              eligible: true, status: "Eligible",
              criteriaName: "DAWN Criteria",
              reason: "Clinical-Core Mismatch",
              details: "Meets DAWN trial inclusion for thrombectomy 6-24h."
          };
      }

      // DEFUSE 3 Criteria Logic
      // Core < 70, Mismatch Vol >= 15, Ratio >= 1.8
      if (!isNaN(mmVol) && !isNaN(ratio)) {
          if (core < 70 && mmVol >= 15 && ratio >= 1.8) {
              return {
                  eligible: true, status: "Eligible",
                  criteriaName: "DEFUSE-3 Criteria",
                  reason: "Perfusion Mismatch",
                  details: "Meets DEFUSE-3 trial inclusion (Core < 70ml, Ratio ≥ 1.8)."
              };
          }
      }

      // SELECT2 / ANGEL-ASPECT (Large Core)
      // If DAWN/DEFUSE failed, check if they qualify under Large Core criteria
      // SELECT2 allowed Core >= 50ml. ANGEL-ASPECT allowed Core 70-100ml.
      // We check for Core >= 50ml.
      if (core >= 50) {
           const details = core > 100 
            ? "Core > 100ml. Benefit shown in ANGEL-ASPECT (up to 100ml) and SELECT2. Very large cores (>130ml) may be futile. CAUTION: Requires strict risk/benefit discussion and multidisciplinary consensus."
            : "SELECT2 and ANGEL-ASPECT trials show improved functional outcomes for large core strokes (Core ≥ 50ml). CAUTION: Higher hemorrhage risk. Requires multidisciplinary discussion and risk vs. benefit conversation.";
      ...
      return {
          eligible: false, status: "Not Eligible",
          reason: "No Target Profile",
          details: "Does not meet DAWN/DEFUSE-3 (Mismatch) or SELECT2/ANGEL-ASPECT (Large Core) criteria.",
          exclusionReason: "Imaging criteria not met."
      };
  }
...
const SelectionCard = React.memo(({ title, description, selected, onClick, variant = 'default' }: SelectionCardProps) => (
    ...
    >
        <div className="flex items-start justify-between relative z-10">
            <div>
                <span className={`block font-bold ${selected ? 'text-current' : 'text-slate-900'}`}>{title}</span>
                {description && <span className={`text-sm mt-1 block ${selected ? 'opacity-90' : 'text-slate-500'}`}>{description}</span>}
            </div>
            {selected && (
                <div className={`p-1 rounded-full ${variant === 'danger' ? 'bg-red-500 text-white' : 'bg-neuro-500 text-white'}`}>
                    <Check size={14} />
                </div>
            )}
        </div>
...
const EvtPathway: React.FC = () => {
  ...
  useEffect(() => {
    setResult(calculateEvtProtocol(inputs));
  }, [inputs]);
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center space-x-3 mb-2">
            <div className="p-2 bg-neuro-100 text-neuro-700 rounded-lg">
                <Zap size={24} className="fill-neuro-700" />
            </div>
            ...
            <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                <div ref={el => { fieldRefs.current['lvo'] = el; }}>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                         <SelectionCard 
                            title="LVO Confirmed?"
                            description="Occlusion of ICA or MCA (M1) on CTA/MRA."
                            selected={inputs.lvo === 'yes'}
                            onClick={() => updateInput('lvo', 'yes')}
                         />
                         <SelectionCard 
                            title="No LVO / Distal Only"
                            description="Standard criteria require proximal LVO."
                            selected={inputs.lvo === 'no'}
                            onClick={() => updateInput('lvo', 'no')}
                         />
                    </div>
                </div>
                ...
                {inputs.lvo === 'yes' && (
                    <div ref={el => { fieldRefs.current['mrs'] = el; }} className="animate-in fade-in slide-in-from-top-2">
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            <SelectionCard 
                                title="Independent (mRS 0-1)"
                                description="No symptoms or no significant disability prior to stroke."
                                selected={inputs.mrs === 'yes'}
                                onClick={() => updateInput('mrs', 'yes')}
                            />
                            <SelectionCard 
                                title="Dependent (mRS > 1)"
                                description="Requires assistance for ADLs prior to stroke."
                                selected={inputs.mrs === 'no'}
                                onClick={() => updateInput('mrs', 'no')}
                            />
                        </div>
                    </div>
                )}
                ...
                {inputs.mrs === 'yes' && (
                     <div ref={el => { fieldRefs.current['age'] = el; }} className="animate-in fade-in slide-in-from-top-2">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            ...
                            <button onClick={() => updateInput('age', '18_79')} className={`p-4 rounded-xl border-2 font-bold transition-all ${inputs.age === '18_79' ? 'border-neuro-500 bg-neuro-50 text-neuro-900' : 'bg-white border-gray-100'}`}>18 - 79</button>
                            <button onClick={() => updateInput('age', '80_plus')} className={`p-4 rounded-xl border-2 font-bold transition-all ${inputs.age === '80_plus' ? 'border-neuro-500 bg-neuro-50 text-neuro-900' : 'bg-white border-gray-100'}`}>≥ 80</button>
                        </div>
                     </div>
                )}

                {isTriageFail && (
                     <div className="bg-red-50 border border-red-100 p-4 rounded-xl flex items-start space-x-3 text-red-800 animate-in zoom-in-95 mt-4">
                         <XCircle className="flex-shrink-0 mt-0.5" size={20} />
                         <div>
                             <p className="font-bold">Likely Ineligible</p>
                             <p className="text-sm mt-1">{result?.exclusionReason || "Does not meet standard screening criteria."}</p>
                         </div>
                     </div>
                )}
            </div>
        ...
        {step === 2 && (
            <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                <div ref={el => { fieldRefs.current['time'] = el; }}>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        <SelectionCard 
                            title="Early Window (0 - 6 Hours)"
                            description="Uses NCCT (ASPECTS) or CTP."
                            selected={inputs.time === '0_6'}
                            onClick={() => updateInput('time', '0_6')}
                        />
                        <SelectionCard 
                            title="Late Window (6 - 24 Hours)"
                            description="Requires Perfusion Imaging (DAWN/DEFUSE-3)."
                            selected={inputs.time === '6_24'}
                            onClick={() => updateInput('time', '6_24')}
                        />
                    </div>
                </div>
                ...
                {inputs.time === '0_6' && (
                    <div ref={el => { fieldRefs.current['aspects'] = el; }}>
                        ...
                        <input 
                            ...
                            className="w-full p-4 text-lg bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-neuro-500 outline-none font-bold"
                            placeholder="e.g. 8"
                    ...
                    <div>
                        ...
                        <div className="space-y-4">
                            <div ref={el => { fieldRefs.current['core'] = el; }}>
                                ...
                                <input 
                                    type="number"
                                    className="w-full p-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-neuro-500 outline-none font-medium"
                                    placeholder="CBF < 30%"
                                    value={inputs.core}
                                    onChange={(e) => updateInput('core', e.target.value)}
                                />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div ref={el => { fieldRefs.current['mismatchVol'] = el; }}>
                                    ...
                                    <input 
                                        type="number"
                                        className="w-full p-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-neuro-500 outline-none font-medium"
                                        placeholder="Tmax > 6s volume"
                                        value={inputs.mismatchVol}
        ...
        {step === 4 && result && (
             <div className="space-y-6 animate-in zoom-in-95 duration-300">
                <div className={`p-8 rounded-3xl relative overflow-hidden shadow-xl text-white ${
                    result.eligible 
                        ? 'bg-slate-900' 
                        : result.status === 'Consult' ? 'bg-amber-500' : 'bg-red-600'
                }`}>
                    <div className="absolute top-0 right-0 w-40 h-40 bg-white opacity-10 rounded-full blur-3xl -mr-10 -mt-10"></div>
                    <div className="relative z-10">
                        <div className="inline-flex items-center space-x-2 bg-white/10 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider mb-4">
                           <Activity size={12} />
                           <span>Recommendation</span>
                        </div>
                        
                        <div className="mb-6">
                            <div className="text-4xl font-black tracking-tight">{result.status}</div>
                            {result.criteriaName && <div className="text-lg font-medium opacity-90 mt-1">{result.criteriaName}</div>}
                        </div>
                        ...
                        <div className="pt-6 border-t border-white/20">
                             ...
                             <div className="text-sm opacity-90 mt-1 leading-relaxed">{result.details}</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                     ...
                     <ul className="space-y-3 text-sm text-slate-700 font-medium">
                        <li className="flex justify-between border-b border-gray-50 pb-2">
                            ...
                            <span className="font-bold">{inputs.time === '0_6' ? '0 - 6 Hours' : '6 - 24 Hours'}</span>
                        </li>
                        <li className="flex justify-between border-b border-gray-50 pb-2">
                            <span>LVO Status</span>
                            <span className="font-bold">{inputs.lvo === 'yes' ? 'Confirmed' : 'None'}</span>
                        </li>
                        <li className="flex justify-between border-b border-gray-50 pb-2">
                            <span>NIHSS</span>
                            <span className="font-bold">{inputs.nihss.replace('_', '-').replace('plus', '+')}</span>
                        </li>
                        {inputs.time === '0_6' && (
      ...
      <div id="evt-action-bar" className="mt-8 pt-4 border-t border-gray-100 scroll-mt-4">
         <div className="flex items-center justify-between gap-4">
             ...
             {step < 4 ? (
                 <button 
                    onClick={handleNext} 
                    className="px-8 py-3 bg-neuro-600 text-white rounded-xl font-bold hover:bg-neuro-700 shadow-lg shadow-neuro-200 transition-all flex items-center transform active:scale-95"
                 >
                 ...
                 <button onClick={copySummary} className="px-8 py-3 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 shadow-lg transition-all flex items-center transform active:scale-95">
                     <Copy size={16} className="mr-2" /> Copy to EMR
                 </button>
             )}
         </div>
         {step === 4 && (
